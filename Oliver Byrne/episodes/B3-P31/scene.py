...
class ByrneIII31(Scene):
    ...
    def construct(self):
        ...
        def perpendicular_bisector_endpoints(chord_end):
            midpoint = 0.5 * (moving_point() + chord_end)
            outward = midpoint - center
            outward /= np.linalg.norm(outward)
            return center, midpoint + 0.62 * outward
        ...
        left_bisector_start, left_bisector_end = perpendicular_bisector_endpoints(left)
        left_bisector = DashedLine(
            left_bisector_start,
            left_bisector_end,
            color=red,
            stroke_width=4,
        ...
        def update_left_bisector(m):
            start, end = perpendicular_bisector_endpoints(left)
            m.become(
                DashedLine(
                    start,
                    end,
        ...
        left_bisector.add_updater(update_left_bisector)

        right_bisector_start, right_bisector_end = perpendicular_bisector_endpoints(right)
        right_bisector = DashedLine(
            right_bisector_start,
            right_bisector_end,
            color=red,
            stroke_width=4,
            dash_length=0.10,
        ...
        def update_right_bisector(m):
            start, end = perpendicular_bisector_endpoints(right)
            m.become(
                DashedLine(
                    start,
                    end,
        ...
        right_bisector.add_updater(update_right_bisector)
